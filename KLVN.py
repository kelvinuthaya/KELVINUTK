...
st.markdown("""
    <style>
        ...
        @media screen and (max-width: 768px) {
            ...
            /* Rendre l'iframe du CV responsive */
            iframe {
                width: 100% !important;
                height: 500px; /* Ajuster la hauteur selon tes préférences */
            }
        }
    </style>
""", unsafe_allow_html=True)

# Vérification du mot de passe dans le session_state
if 'mot_de_passe_valide' not in st.session_state:
    st.session_state.mot_de_passe_valide = False

# Demander un mot de passe si celui-ci n'est pas valide
if not st.session_state.mot_de_passe_valide:
    st.title("Entrer le mot de passe")
    mot_de_passe = st.text_input("Mot de passe", type="password")

    if st.button("Valider"):
        if mot_de_passe == "kelvin2893":
            st.session_state.mot_de_passe_valide = True
            st.success("Mot de passe correct, vous pouvez accéder au contenu !")
        else:
            st.error("Mot de passe incorrect. L'accès est limité.")
else:
    # Affichage de la page principale une fois le mot de passe validé
    ...
    st.markdown(f'<iframe src="{CV_VIEWER_URL}" width="100%" height="800"></iframe>', unsafe_allow_html=True)
